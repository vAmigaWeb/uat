...
const ui_version = '2026_08_15b'+url_root_path.replace("/","_"); 
...
self.addEventListener('activate', evt => {
  ...
  let check_and_update=async () =>{    
    ...
    else if(current_version != cache_name)
    {
      ...
      if (core_v < "4.3.6") {
        //when older than 436 non aga release, force potential crashed install to use the new version
        //which now has implemented some validation checks, to disable AGA which is not supported by these old versions
        await set_settings_cache_value("active_version", cache_name);
      }
    }
  }
